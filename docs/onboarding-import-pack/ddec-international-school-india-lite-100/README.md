# DDEC International School — Lite 100 onboarding pack

Lightweight onboarding set (same conventions as `DDEC` 300-pack imports):
...
- `02_teachers.csv`: `33` rows + header; each row has distinct name + credential-style qualification/specialization (pool subject still matches timetable); emails `firstname.lastname.t###@ddecinternational.edu.in`.
- `03_staff.csv`: `5` operations staff rows.
- `04_students.csv`: `100` admissions; guardian emails `guardian.<admission>-{seq}@ddecinternational.edu.in`.
