...
public class TimetableService {
    ...
    public TimetableDTOs.TimetableGridResponse getGrid(Long classId, Long sectionId) {
        ...
        List<String> days = entries.stream().map(e -> e.getDay().name()).distinct().sorted(Comparator.comparingInt(d -> List.of("MONDAY", "TUESDAY", "WEDNESDAY", "THURSDAY", "FRIDAY", "SATURDAY").indexOf(d))).collect(Collectors.toList());
        ...
        for (String day : days) {
            ...
            for (int period : periods) {
                TimetableEntry entry = entries.stream().filter(e -> e.getDay().name().equals(day) && e.getPeriod() == period).findFirst().orElse(null);
                if (entry != null) {
                    daySlots.put(period, TimetableDTOs.SlotDTO.builder().subject(entry.getSubjectName()).teacher(entry.getTeacherName()).room(entry.getRoom()).startTime(entry.getStartTime() != null ? entry.getStartTime().toString() : null).endTime(entry.getEndTime() != null ? entry.getEndTime().toString() : null).build());
                }
            }
