...
public class SettingsController {
    ...
    @GetMapping @Operation(summary = "Get school settings")
    public ResponseEntity<ApiResponse<TenantConfig>> get() {
        return ResponseEntity.ok(ApiResponse.ok(service.getSettings()));
    }

    @PutMapping @PreAuthorize("hasRole('ADMIN')") @Operation(summary = "Update school settings", description = "Update name, address, phone, email, branding colors, logo")
    ...
    @GetMapping("/features") @Operation(summary = "Get feature flags", description = "Returns enabled/disabled status for each module")
    ...
    @PutMapping("/features") @PreAuthorize("hasRole('ADMIN')") @Operation(summary = "Update feature flags", description = "Enable/disable modules: transport, library, hostel, payroll, etc.")
