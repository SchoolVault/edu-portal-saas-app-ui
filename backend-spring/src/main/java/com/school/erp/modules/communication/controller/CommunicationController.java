...
public class CommunicationController {
    ...
    public ResponseEntity<ApiResponse<Announcement>> createAnnouncement(@RequestBody Announcement ann) {
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.created(service.createAnnouncement(ann)));
    }

    @PutMapping("/announcements/{id}") @PreAuthorize("hasAnyRole('ADMIN','TEACHER')") @Operation(summary = "Update announcement")
    ...
    public ResponseEntity<ApiResponse<Void>> deleteAnnouncement(@PathVariable Long id) {
        service.deleteAnnouncement(id); return ResponseEntity.ok(ApiResponse.ok(null, "Deleted"));
    }

    // --- Teacher-Parent Messaging ---
    @GetMapping("/messages") @Operation(summary = "Get my messages", description = "All messages where current user is sender or receiver")
    ...
    @GetMapping("/messages/conversation/{otherUserId}") @Operation(summary = "Get conversation with a user")
    ...
    @PostMapping("/messages") @Operation(summary = "Send message", description = "Send message to another user (teacher-parent communication)")
    ...
    @PutMapping("/messages/{id}/read") @Operation(summary = "Mark message as read")
    public ResponseEntity<ApiResponse<Void>> markRead(@PathVariable Long id) {
        service.markMessageRead(id); return ResponseEntity.ok(ApiResponse.ok(null));
    }

    @GetMapping("/messages/unread-count") @Operation(summary = "Get unread message count")
